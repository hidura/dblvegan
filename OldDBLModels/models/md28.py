...
class AccountMove(models.Model):
    ...
    @api.depends('line_ids', 'line_ids.price_subtotal', 'state', 'invoice_payment_state')
    ...
    @api.depends('invoice_line_ids', 'invoice_line_ids.product_id', 'state', 'invoice_payment_state')
    ...
    @api.depends('line_ids', 'line_ids.product_id', 'state', 'invoice_payment_state')
    ...
    def _compute_in_invoice_payment_form(self):
        for inv in self:
            ...
            elif inv.invoice_payment_state == 'paid':
            ...
            # Retenciones en la factura
                ...
                if inv_type == 'out_invoice':
                    _logger.info(('Retenciones Factura 607', [move_line.debit
                                                              ...
                                                              if
                                                              move_line.account_id.account_fiscal_type in witheld_type]))
    ...
    @api.depends('line_ids.tax_line_id', 'state', 'invoice_payment_state')
                ...
                if inv.type == 'out_invoice':
                    ...
                    for line in tax_line_ids:
                        ...
                        inv.third_withheld_itbis += sum([line.debit
                                                         if line.account_id.account_fiscal_type in ['A34',
                                                                                                    'A36'] else 0]
                                                        )

                        # Retención de Renta pr Terceros
                        inv.third_income_withholding += sum([line.debit
                                                             if line.account_id.account_fiscal_type in ['ISR',
                                                                                                        'A38'] else 0]
                                                            )
    ...
    @api.depends('invoiced_itbis', 'cost_itbis', 'state', 'invoice_payment_state')
