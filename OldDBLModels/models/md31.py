    ...
    def _compute_607_fields(self):
        for rec in self:
            ...
            for inv in sale_line_ids:
                ...
                data['sale_records'] += 1
                data[
                    'sale_invoiced_amount'] += inv.invoiced_amount if inv.invoice_id.type == 'out_invoice' else -inv.invoiced_amount
                data[
                    'sale_invoiced_itbis'] += inv.invoiced_itbis if inv.invoice_id.type == 'out_invoice' else -inv.invoiced_itbis
                data[
                    'sale_withholded_itbis'] += inv.third_withheld_itbis if inv.invoice_id.type == 'out_invoice' else -inv.third_withheld_itbis
                data[
                    'sale_withholded_isr'] += inv.third_income_withholding if inv.invoice_id.type == 'out_invoice' else -inv.third_income_withholding
                data[
                    'sale_selective_tax'] += inv.selective_tax if inv.invoice_id.type == 'out_invoice' else -inv.selective_tax
                data[
                    'sale_other_taxes'] += inv.other_taxes if inv.invoice_id.type == 'out_invoice' else -inv.other_taxes
                data['sale_legal_tip'] += inv.legal_tip if inv.invoice_id.type == 'out_invoice' else -inv.legal_tip
