...
class DgiiReport(models.Model):
    ...
    def _get_invoices(self, states, types):
        ...
        :param states: a list of invoice state
        :param types: a list of invoice type
    ...
    def _include_in_current_report(self, invoice):
        ...
        :return: boolean
        """
        if not invoice.payment_date or not any([
            invoice.third_withheld_itbis, invoice.third_income_withholding, invoice.withholded_itbis, invoice.income_withholding
        ]):
            return False
    ...
    def _compute_606_data(self):
        for rec in self:
            ...
            invoice_ids = self._get_invoices(['posted'],
                                             ['in_invoice', 'in_refund']).filtered(
                lambda inv: inv.l10n_latam_document_type_id.l10n_do_ncf_type != 'exterior'
            )
    ...
    def _convert_to_user_currency(self, invoice_id, amount):
        rate = invoice_id.amount_total_signed / invoice_id.amount_total
        return rate * amount
    ...
    def _get_sale_payments_forms(self, invoice_id):
        ...
        if invoice_id.move_type == 'out_invoice':
            # Here we check is point_of_sale is installed and invoice has pos_order_ids populated
            if hasattr(invoice_id, "pos_order_ids") and getattr(invoice_id, "pos_order_ids"):
                payments = invoice_id.pos_order_ids and invoice_id.pos_order_ids[0].payment_ids or False
                ...
                for payment in invoice_id._get_invoice_payment_widget():
                    ...
                    elif payment.get('move_id', False):
                        refund_id = self.env['account.move'].search([
                            ('id', '=', payment.get('move_id')), ('move_type', '=', 'out_refund')
                        ])
                        if refund_id:
                            payments_dict['credit'] += self._convert_to_user_currency(
                                invoice_id, abs(payment['amount']))

            payments_dict['credit'] += self._convert_to_user_currency(
                invoice_id, invoice_id.amount_residual)
    ...
    def _process_op_dict(self, args, invoice):
        ...
        if invoice.move_type == 'out_refund' and invoice.debit_note_count == 0:
    ...
    def _has_withholding(self, inv):
        """Validate if given invoice has a Withholding tax"""
    ...
    def _invoice_status_sent(self):
        for report in self:
            ...
            for inv in invoice_ids:
                if (inv.payment_state in ['paid'] or inv.state in ['cancel']) and \
